...
class AutoCompleter:
    def __init__(self, builtins: dict, executables: List[str]):
        readline.parse_and_bind("tab: complete")
        readline.set_completer(self.completer)

        self.matches = []
        self.commands = sorted(
            set(list(builtins.keys()) + executables), key=lambda x: (len(x), x)
        )
...
def main():
    ...
    AutoCompleter(
        builtins=builtins, executables=sys.executable_finder.get_executables()
    )
    while True:
        line = input("$ ").strip()
        if not line:
